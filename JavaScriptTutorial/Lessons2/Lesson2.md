...
1. 含义：比较两个值是否相等，不管数据类型 (注意！因为会隐式转换类型)
2. 例子：
...
对象（Object）是 JavaScript 中最重要的数据类型之一，它用于存储**键值对（key-value pair）**的集合。
...
对象里面不仅可以存数据，还可以存**函数**。对象里面的函数叫做**方法（Method）**。
...
// 方法二：检查是否为 undefined
console.log(person.city !== undefined); // false（不存在）
console.log(person.name !== undefined); // true（存在）
```
...
| -------- | ------------------------ | ------------------------- |
| 本质     | 有序的数据集合           | 键值对的集合              |
| 访问方式 | 通过索引（数字，0 开始） | 通过属性名（字符串）      |
| 是否有序 | 有序                     | 无序（通常按添加顺序）    |
| 适用场景 | 同类型数据的列表         | 描述一个事物的各种属性    |
| 示例     | `["apple", "banana"]`    | `{name: "张三", age: 20}` |
